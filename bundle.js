...
fetch("notebook.json").then((r) => {
  ...
  let sParam = null, pParam = null, pHint = null, navX = null, navY = null;
  ...
  if (hash.startsWith("#!/")) {
    ...
    if (parts[1]) {
      ...
      if (hp.has("p"))
        pHint = hp.get("p");
      if (hp.has("x"))
        navX = parseFloat(hp.get("x"));
      if (hp.has("y"))
        navY = parseFloat(hp.get("y"));
    }
  } else {
    ...
    if (nb) {
      ...
      if (sec) {
        ...
        if (pParam || pHint) {
          ...
          var findPage = findPage2;
          const pg = findPage2(sec.pages);
          if (pg) {
